...
def test_create_hardlinks():
    ...
    vhpi.snapshot.create_hardlinks(src, dst)
    after_dst_is_dir = os.path.isdir(dst)
    assert after_dst_is_dir is True
...
def test_shift():
    ...
    vhpi.snapshot.shift('hourly', parent_dir)
...
def test_make():
    ...
    for snapshot in glob.glob(parent + '/hourly.*'):
        shutil.rmtree(snapshot)
    vhpi.snapshot.make('hourly', src)
    vhpi.snapshot.make('hourly', src)
    vhpi.snapshot.make('hourly', src)
